...
instance TacPP TAC where
      ...
      prettyPrint(TACNewTmpCast temp typ1 genTyp addr1) = nest tab $ text temp<+> text "="<+> text "Cast"<+>text (show typ1) <+> text "To"<+>text (show genTyp)<+>text addr1
      ...
      prettyPrint (TACInitCast typ id pos  exp)= case exp of
        ...
        Just exp->nest tab $ text id<>text("_"++(show pos)) <+> text "="<+>text(show typ)<+>text exp 
