...
render d = rend 0 (map ($ "") $ d []) "" where
  rend i ss = case ss of
    ...
    "try"      :ts -> showString "try" . new (i+1) . rend (i+1) ts
    "catch"      :ts -> showString "catch" . new (i+1) . rend (i+1) ts
...
instance Print Dec where
  prt i e = case e of
    ...
    VarDeclar mod typespecifier pident exp -> prPrec i 0 (concatD $ prmod++[prt 0 typespecifier,prt 0 pident]++ prex++[nl])
       where prex=case exp of Nothing -> [] ; (Just exp) -> [doc (showString "="),prt 0 exp]
             prmod =case mod of Nothing -> [] ; (Just modality) -> [prt 0 modality]
  prtList _ [] = (concatD [])
...
instance Print Stm where
  prt i e = case e of
    ...
    Break _ -> prPrec i 0 (concatD [doc (showString "break"),nl])
    Continue  _-> prPrec i 0 (concatD [doc (showString "continue"),nl])
  prtList _ [] = (concatD [])
...
instance Print Exp where
  prt i e = case e of
    ...
    TernaryOp exp1 exp2 exp3->prPrec i 0 (concatD [prt 0 exp1,doc (showString "?"), prt 0 exp2,doc (showString ":"), prt 0 exp3])
